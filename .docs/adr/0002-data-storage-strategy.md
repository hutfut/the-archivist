...
- **Tests use testcontainers PostgreSQL**: Tests run against a real pgvector PostgreSQL container via `testcontainers-python`, matching the production database engine. This eliminates SQLite/PostgreSQL behavioral differences and allows testing pgvector columns directly. Docker is required to run the test suite.
