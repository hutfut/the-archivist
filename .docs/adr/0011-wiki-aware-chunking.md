...
3. **Horizontal rules as hard boundaries** — Treat standalone `---`, `***`, `___` as split points so major wiki blocks become separate chunks. Implemented as an optional second step: pre-split on HR, then run header split on each block. Bold-line or list-aware separators deferred.

4. **List-aware or custom separators** — Split on list boundaries or other wiki conventions. Deferred: more complex and corpus-specific; header + size + HR cover the main structural cases.
...
- **Horizontal rules** (implemented): Pre-split markdown on lines that are only `---`, `***`, or `___` (optional surrounding whitespace), then run existing header splitter on each block. Chunks from HR-separated blocks get their heading from the header hierarchy within that block (no cross-block heading inheritance; each block is processed independently so headings are parsed normally).
...
- No new runtime dependencies; LangChain text splitters and stdlib (e.g. regex) only.
